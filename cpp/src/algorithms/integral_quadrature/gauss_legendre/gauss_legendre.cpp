...
double gauss_legendre_price(double spot, double strike, double t, double vol,
                            ...
                            const GaussLegendreParams& params) {
    ...
    if (n_points < 8 || n_points > 256 || params.integration_limit <= 0.0) {
        return detail::nan_value();
    }

    std::vector<double> nodes;
    std::vector<double> weights;
    if (!detail::gauss_legendre_rule(n_points, nodes, weights)) return detail::nan_value();

    double log_moneyness = std::log(spot / strike);
    double u_max = params.integration_limit;
    double integral = 0.0;

    for (int32_t i = 0; i < n_points; ++i) {
        double x = nodes[static_cast<std::size_t>(i)];
        double u = 0.5 * (x + 1.0) * u_max;
        double mapped_w = 0.5 * u_max * weights[static_cast<std::size_t>(i)];
        integral += mapped_w * detail::lewis_integrand(u, log_moneyness, t, vol, r, q);
    }
