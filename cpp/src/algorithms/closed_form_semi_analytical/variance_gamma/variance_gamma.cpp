...
double variance_gamma_price_cf(double spot, double strike, double t, double r, double q,
    ...
    double drift_arg = 1.0 - params.theta * params.nu - 0.5 * params.sigma * params.sigma * params.nu;
    ...
    auto cf = [&](std::complex<double> u) -> std::complex<double> {
        ...
        std::complex<double> base = 1.0 - params.theta * params.nu * iu
            + 0.5 * params.sigma * params.sigma * params.nu * u * u;
        std::complex<double> drift = std::exp(iu * (log_spot + (r - q + omega) * t));
        std::complex<double> jump = std::pow(base, -t / params.nu);
        return drift * jump;
    };
